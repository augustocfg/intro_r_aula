Edgeworth <- function(Q1_h = NULL, E_h = c(10,0), E_f = c(0,10), alpha = 0.5, beta = 0.5, contract = TRUE, budget = TRUE, mrs = FALSE, solution = TRUE, ...) {
  E  <- E_h + E_f
  E1 <- E[1]; E2 <- E[2] 
  a  <- alpha/(1-alpha)
  b  <- beta/(1-beta)
  ...
  if(solution) {
    ...
    p<- p +
      ...
      #                                           ((E1-x)^beta))^(1/(1-beta)), 
      #               args = list(i = i), color = 'black') + 
      geom_point(aes(x = Q1_h_star, y = Q2_h_star), color = 'darkgreen',size= 3) +
